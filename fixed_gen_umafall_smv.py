...
for path, dirs, files in os.walk(dir_path):
    ...
    for file in files:
        ...
        for j in range(5):
            ...
            # Kiểm tra kích thước của X_window trước khi tính SMV
            if X_window.shape[0] < window_size:
                print(f"Window size mismatch: expected {window_size}, got {X_window.shape[0]}. Skipping...")
                continue

            # Tính toán Signal Magnitude Vector (SMV)
            X_smv = np.linalg.norm(X_window, axis=1)
            
            # Kiểm tra kích thước sau khi tính toán SMV
            if X_smv.shape[0] != window_size:
                print(f"SMV size mismatch: expected {window_size}, got {X_smv.shape[0]}. Skipping...")
                continue
...
if len(X) > 0:
    ...
    pickle.dump(X, open("data/X_umafall_smv.p", "wb"))
    pickle.dump(df_label, open("data/y_umafall_smv.p", "wb"))
else:
